...
make metrics                                       # strength plus timing/search metrics
make tournament                                    # round-robin Elo leaderboard
make elo                                           # 20-game-per-pair Elo evaluation
make test                                          # legal-move and edge-case smoke tests
...
`make metrics` writes `metrics.json` and reports wins, draws, losses, score, an Elo estimate
relative to a nominal 1500-rated opponent, per-move latency (mean, p50, p95, maximum),
average nodes, and completed search depth. For version comparisons, keep an older agent in a
directory and run:

```bash
uv run python -m tools.metrics --opponent ../my-old-version --games 200 --json-out new.json
```

The Elo estimate is meaningful only for comparisons using the same opponent and time control.
Use alternating colours and enough games to distinguish a real improvement from variance.

For a local Stockfish comparison, keep the executable outside the submission and add it to the
round robin:

```bash
uv run python -m tools.tournament . baselines/minimax \
  --stockfish /path/to/stockfish \
  --games-per-pair 20 --json-out tournament.json
```

Stockfish is used only as a local benchmark here; it must not be imported or shipped in `agent.py`.

The standard future evaluation is:

```bash
make elo
```

To include the local Stockfish benchmark:

```bash
make elo STOCKFISH=/path/to/stockfish
```

This produces `elo.json`. Keep the clock, agent list, and games-per-pair fixed when comparing
search or time-management changes.
